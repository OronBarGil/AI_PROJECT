...
class DecisionTree():
    ...
    def _gini_impurity(self, labels: list) -> float:
        """Calculate gini impurity - often works better than entropy for images"""
        if not labels.any():
            return 0.0
    ...
    def _partition_gini(self, subsets: list) -> float:
        """Calculate weighted gini impurity for a partition"""
        total_count = sum([len(subset) for subset in subsets])
        return sum([self._gini_impurity(subset) * (len(subset) / total_count) for subset in subsets])
    ...
    def _find_best_split(self, data: np.array) -> tuple:
        ...
        min_gini_feature_idx = 0
        min_gini_feature_val = 0
        g1_min = None
        g2_min = None
        found_valid_split = False

        # More percentile points for better splitting
        percentiles = np.arange(20, 100, 10)  # Try more split points
        
        for idx in feature_idx_to_use:
            feature_vals = np.percentile(data[:, idx], q=percentiles)
        ...
        if not found_valid_split:
            # Create a simple split if no valid split was found
            # This fallback simply divides the data in half
            mid = len(data) // 2
            g1_min = data[:mid]
            g2_min = data[mid:]
            min_gini_feature_idx = 0
            min_gini_feature_val = 0
            min_part_gini = self._partition_gini([g1_min[:, -1], g2_min[:, -1]])

        # Calculate information gain as 1 - gini (for compatibility)
        node_gini = self._gini_impurity(data[:, -1])
        information_gain = node_gini - min_part_gini

        return g1_min, g2_min, min_gini_feature_idx, min_gini_feature_val, information_gain
